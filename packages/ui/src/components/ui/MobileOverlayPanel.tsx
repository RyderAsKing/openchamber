...
interface MobileOverlayPanelProps {
  ...
  /** 'partial' opens at ~70% height and expands on interaction; 'full' keeps the legacy sizing. */
  detent?: 'partial' | 'full';
}
...
const PARTIAL_PANEL_HEIGHT = '70dvh';
...
export const MobileOverlayPanel: React.FC<MobileOverlayPanelProps> = ({
  ...
  const contentMaxHeight = contentMaxHeightClassName ?? (detent === 'partial' ? 'max-h-full' : 'max-h-[min(70vh,520px)]');
  const usesPartialDetent = detent === 'partial';
  const fullPanelHeight = 'calc(100dvh - max(0.75rem, var(--oc-safe-area-top, 0px)))';
  const panelHeight = usesPartialDetent && !expanded ? PARTIAL_PANEL_HEIGHT : fullPanelHeight;
  const panelTransition = usesPartialDetent
    ? `transform ${ENTER_DURATION_MS}ms cubic-bezier(0.32, 0.72, 0, 1), height ${DETENT_TRANSITION_MS}ms cubic-bezier(0.32, 0.72, 0, 1)`
  ...
  const content = (
    ...
    >
        <div
          className={cn(
            'mt-auto flex min-h-0 w-full flex-col rounded-t-xl border-x border-t border-border/50 bg-background shadow-none pwa-overlay-panel',
            'mx-auto max-w-lg',
            className
          )}
          style={{
            height: panelHeight,
            maxHeight: fullPanelHeight,
